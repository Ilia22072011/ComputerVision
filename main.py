...
def blocking_read_img():
    global ret, img, updated_img
...
async def get_img():
    await asyncio.wait(
        fs={
            loop.run_in_executor(executor, blocking_read_img,)
        },
...
async def send():
    while True:
        ...
        #Создаём HSV модели
        ghsv = cv2.cvtColor(img_copy, cv2.COLOR_BGR2HSV )
    
        gthresh = cv2.inRange(ghsv, ghsv_min, ghsv_max)
        rthresh = cv2.inRange(ghsv, rhsv_min, rhsv_max)
        ...
        await asyncio.wait(
            fs={
                loop.run_in_executor(executor, blocking_send, data_to_send)
            },
            return_when=asyncio.ALL_COMPLETED
        )
...
t_send = loop.create_task(send, )
t_get = loop.create_task(get_img, )
